...
This blog recreates the blog post from yesterday, August 3, 2025, so that the contents of the table can be copied from the post.
...
path <- "C:/Users/Owen/OneDrive/Documents/quarto_test/blog/post-20250804/"

dat1 <- readxl::read_excel(paste0(path,"Decomposition_For_Blog_Post.xlsx"),sheet = "All_items")

dat2 <- readxl::read_excel(paste0(path,"Decomposition_For_Blog_Post.xlsx"),sheet = "Eight_indexes")

dat3 <- readxl::read_excel(paste0(path,"Decomposition_For_Blog_Post.xlsx"),sheet = "Eight_groups")
...
gt_table3 <- gt(dat3) |>
  tab_style(
    ...
    locations = cells_body(
      rows = c(3,6,10,16,19,22,30,33,36)
    )
  )
